...
describe("RunsPage", () => {
  ...
  it("renders run detail when plan query present", async () => {
    ...
    const averageScoreLabel = await screen.findByText("Average Score");
  ...
  it("shows average score in the run summary", async () => {
    const fetchMock = vi.fn((url, init) => {
      if (String(url).endsWith("/agent-run-plans/plan-1") && init?.method === "GET") {
        return Promise.resolve({
          ok: true,
        ...
        return Promise.resolve({
          ...
          json: vi.fn().mockResolvedValue({
            items: [
              ...
              { id: "t2", question_index: 1, attempt_index: 0, status: "succeeded", eval_pass: false, score: 0.5, worker_id: "worker-2", input_payload: {}, label_payload: {}, prediction_payload: {} },
            ],
          }),
        });
      }
