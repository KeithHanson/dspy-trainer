...
Restarting the deployer re-observes exact owned containers and resumes the durable phase without duplicating slots. Scale-down retires the highest surplus slots deterministically; deleted deployment intent drains and removes only exact owned containers. Cleanup runs only after all deployment intents converge. It retains at least the newest two ready images per module and never prunes an image referenced as active, target, previous, or by a non-removed managed container; a successful prune marks the build `pruned` durably.
